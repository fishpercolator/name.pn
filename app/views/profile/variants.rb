    ...
    wizard_step do |form|
      ...
      form.nested(:alternate_names, add: t('.add_alternate_name')) do |alternate_name|
        div(class: 'alternate_name-fields') do
          ...
          alternate_name.field :category, as: :select, choices: choices(AlternateName.categories.keys, scope: 'alternate_name_form_categories')
          div(class: 'mt-2 flex justify-end') { alternate_name.remove t('.remove') }
        end
      end
    end
  end
