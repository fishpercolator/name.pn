...
RSpec.describe UserPolicy, type: :policy do
  ...
  context 'being the actual user' do
    ...
    it { is_expected.to permit_mass_assignment_of([:full_name, :personal_name, :formal_name, :envelope_name, :phonetic, :ipa, :slug, :pronunciation_of, :pronoun_set_ids => [], :links_attributes => [:id, :title, :url, :_destroy]]) }
    context 'profile not complete' do
      let(:user_obj) { create :user }
